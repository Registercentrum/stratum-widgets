...
Ext.define('Septum.view.Main', {
      ...
      tooltip: {
          ...
          renderer: function (record, ctx) {
              ...
              if(WidgetConfig.verbose){
                var text = 'Antal: ' + count + '<br/>' + 'Andel (av de som tillfrågats): ' + shareAll + '%';
                text += ctx.field.indexOf('Svar saknas') < 0 ?  '<br/>' + 'Andel (av de som svarat): '      + shareCropped + '%' : '';
              }
              else {
                  var text = 'Antal: ' + count + '<br/>' + 'Andel: ' + shareAll + '%';
              }
              this.setHtml(text);
          }
      },
  ...
  constructor: function (config) {
      ...
      if (!config.restricted) {
          ...
          var fields = fields.map(function (item, index) { return [unitFields[index], item] });
