...
Ext.define('Shpr.chart.Time', {
  ...
  series: [
    ...
    {
      xField: 'year',
      reference: 'comparisonConfidenceInterval',
      type: 'area',
      tooltip: {
        width: 0,
        height: 0,
...
Ext.define('Shpr.controller.Compass', {
  ...
  updateCharts: function () {
    ...
    var compassChart  = view.down('#compassChart')
    var casemixChart  = view.down('#casemixChart')
    var trendChart    = view.down('#trendChart')
    var missingData   = view.down('#missingData')
    var indicator     = view.down('#indicatorFilter').getValue()
    
    var compassurl = controller.createCompassUrl('compass')
    var timeurl = controller.createTrendUrl(indicator)
    ...
    controller.updateTrendSelection()
    controller.updateDependentExplantions()
    controller.disableInvalidFilters()
    ...
    Ext.Ajax.request({
      ...
      success: function (response) {
        ...
        if (result.case_mix) {
          ...
          compassChart.show()
          casemixChart.show()
          compassChart.getStore().loadData(result.compass)
    ...
    Ext.Ajax.request({
      ...
      success: function (response) {
        ...
        controller.togglePercentageIndicator(trendChart, indicator)
        controller.setMinimumForAxis(result, trendChart, indicator)
        controller.toggleConfidenceInterval(result, trendChart)
        controller.hideLoadingIndicator()
        
        trendChart.getStore().loadData(result)
        trendChart.show()
      },
  ...
  findMinimum: function (data) {
    var minimum = data.reduce(this.findMinimumUnitValue, 1)
    minimum = data.reduce(this.findMinimumComparisionValue, minimum)
    return minimum
  },

  findMinimumUnitValue: function (min, current) { 
    return current.y_unit < min ? current.y_unit : min 
  },

  findMinimumComparisionValue: function (min, current) { 
    return current.y_comparison < min ? current.y_comparison : min 
  },
  ...
  createCompassUrl: function () {
    ...
    var url = '/stratum/api/statistics/shpr/kpl_compass?unit=' + unit + '&comparison=' + comparison + '&diagnosis=' + indicator + '&apikey=' + this.apikey
    return url
  },

  createTrendUrl: function (area) {
    var view = this.getView()
    var comparison = view.down('#comparisonFilter').getValue()
    var unit = view.down('#unitFilter').getValue()
    var diagnosis = view.down('#diagnosisFilter').getValue()
  ...
  updateLegend: function (view) {
    ...
    trendChart.getSeries()[2].setTitle(unit || (Profile.Context ? Profile.Context.Unit.UnitName : 'Riket'))
    trendChart.getSeries()[3].setTitle(comparison || 'Riket')
  },
...
Ext.application({
  ...
  start: function () {
    ...
    var main = Ext.create('Shpr.view.Main', { renderTo: target })
    if (!window.navigator.msSaveBlob) {
