...
Repository.Local.Methods.initialize(function(_m) {
    var hypIndicators = ['1016', '1017', '1018', '1019', '1020', '1021'],
        krsIndicators = ['2017', '2018', '2019', '2020', '2021', '2022'],
        clinicComboPrimary, clinicComboSecondary,
        currMonth = _m.getCurrentMonth(),
        currYear = _m.getCurrentYear(),
        startMonth = _m.getStartMonth(),
        startYear = _m.getStartYear(),
        clinicChangeFn, tipChart, tipChart2,
        hypStore, krsStore, tipStore, configPanel, chartConfig;

    krsStore = Ext.create('Ext.data.Store', {
        fields: [{
            name: 'Q_Indicator',
            type: 'string'
        }, 'Q_Month', 'Q_Year', 'Q_Unit_0', 'Q_Varde_0', 'Q_Unit_1', 'Q_Varde_1'],
        sorters: [{
    ...
    tipStore = Ext.create('QRegPV.TipStore', {
        ...
        filters: [
            function(item) {
                return item.get('Q_Year') === startYear ? item.get('Q_Month') >= startMonth :
                    item.get('Q_Year') === currYear && item.get('Q_Month') <= currMonth;
            }
        ]
    });
    var charts = {};
    hypStore = _m.getMainStore({
        beforeLoadFn: function() {
            charts.hypertoni && charts.hypertoni.setLoading('Laddar...');
        ...
        onLoadFn: function(store, records) {
            ...
            krsStore.filterBy(function(item) {
                return Ext.Array.contains(krsIndicators, item.get('Q_Indicator')) && item.get('Q_Month') === currMonth && item.get('Q_Year') === currYear;
            });
        ...
        filter: function(item) {
            return Ext.Array.contains(hypIndicators, item.get('Q_Indicator')) && item.get('Q_Month') === currMonth && item.get('Q_Year') === currYear;
        },
        sorters: [{
            property: 'Q_Indicator',
            direction: 'DESC'
    ...
    Ext.each(chartConfig, function(chart) {
        charts[chart.chartName] = Ext.create('Ext.chart.CartesianChart', {
            colors: chart.colors,
            plugins: {
            ...
            series: [{
                ...
                listeners: {
                    itemmouseover: function(series, item) {
                        var indicator = item.record.get('Q_Indicator');
