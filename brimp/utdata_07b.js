...
(function() {
	...
	var baseUrl = '/api/statistics/brimp/utdata_07b?rinvoke=1&unitId={0}';
	...
	var transformData = function(arrData) {
		var result = {
			...
			labels:[], 
			fields: ['a','b','c','d'], 
			fieldLabels: ['al','bl','cl','dl'],
			fieldTotals: ['at','bt','ct','dt'],
			fieldPercents: ['ap','bp','cp','dp'], 
			data:[] 
		};
	...
	Ext.Ajax.request({
		...
		success: function(response, opts) {
			...
			if (arrData.length === 0)
				...
				Ext.fly('utdata').update('<p>Andel patienter som fick antibiotika och andel patient som inte fick antibiotika per indikation vid primäroperation för ' + unitName + '.</p>');
			var myData = transformData(arrData);
			var myStore = createStore(myData.data);
			var header = 'Antibiotika per indikation vid primäroperation';
