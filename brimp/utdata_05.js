...
(function() {
	...
	var baseUrl = '/api/statistics/brimp/utdata_05?rinvoke=1&unitId={0}';
	...
	var transformData = function(arrData) {
		var result = { 
			...
			labels:[], 
			fields: ['a','b','c','d','e'], 
			fieldLabels: ['al','bl','cl','dl','el'],
			fieldTotals: ['at','bt','ct','dt','et'],
			fieldPercents: ['ap','bp','cp','dp','ep'], 
			data:[] 
		};
	...
	var createChart = function(header, fields, fieldPercents, labels, store) {
		return Ext.create('Ext.chart.Chart', {
			...
			animation: false,
			border: false,
			colors: ['#A2AD00', '#614D7D', '#3CB6CE', '#E98300', '#8497B0'],
			background: '#fff',
			title: header,
			style: {
				borderWidth: '1px',
				...
				tooltip: {
					...
					renderer: function(rec, item) {
						...
						if (rec.data.isNA === true) {
							...
							text += Ext.String.format('{0}: {1} ({2} av {3})', rec.data.el, Ext.util.Format.number(rec.data.ep, '0.0%'), rec.data.e, rec.data.et);
						}
						this.setHtml(text);
					}
				},
	...
	Ext.Ajax.request({
		...
		success: function(response, opts) {
			...
			if (arrData.length === 0)
				...
				Ext.fly('utdata').update('<p>Åldersfördelning per indikation vid primäroperation för ' + unitName + '.</p>');
			var myData = transformData(arrData);
			var myStore = createStore(myData.data);
			var header = 'Åldersfördelning per indikation vid primäroperation';
