...
(function() {
	...
	var baseUrl = '/api/statistics/brimp/utdata_08?rinvoke=1&unitId={0}';
	...
	var transformData = function(arrData) {
		var result = {
			...
			labels:[], 
			fields: ['a','b','c'], 
			fieldLabels: ['al','bl','cl'],
			fieldTotals: ['at','bt','ct'],
			fieldPercents: ['ap','bp','cp'], 
			data:[] 
		};
				...
				tooltip: {
					...
					renderer: function(tooltip, rec, item) {
						...
						if (rec.data.isNA === true) {
							...
							text += Ext.String.format('{0}: {1} ({2} av {3})', rec.data.cl, Ext.util.Format.number(rec.data.cp, '0.0%'), rec.data.c, rec.data.ct);
						}
	...
	Ext.Ajax.request({
		...
		success: function(response, opts) {
			...
			if (arrData.length === 0)
				...
				Ext.fly('utdata').update('<p>Patientupplevelse innan primäroperation för ' + unitName + '.</p>');
			var myData = transformData(arrData);
			var myStore = createStore(myData.data);
			var header = 'Patientupplevelse innan primäroperation';
