...
(function(aProfileName) {
	var container = Stratum.containers && Stratum.containers['RC/SubjectOverview'] || 'SubjectOverviewContainer',
		SO = Repository.Local.Methods[isTemplate(aProfileName) ? 'SubjectOverview' : aProfileName],
		data; // Holds both feed from subject overview and statistics gathered about this data.
	
	function getSubjectHeading() {
		return [{ header: 'Person', dataIndex: 'SubjectKey', width: 120, /*locked: true,*/ menuDisabled: true }]
	}
	...
	function loadStyleSheet() {
		...
		Ext.util.CSS.createStyleSheet('' +
			...
			'@media print { td { border: 1px solid black; } }', 'sow');
	}
	...
	function createSummaryBar() {
		var dd = Ext.Date.format(new Date(), 'Y-m-d');
	...
	function createDateFilter() {
		var filter = Ext.widget('toolbar', {
			...
			filterStore: function(aCallback, button, text) {
				var table = this.up().down('toolbar');
				
				var store = Ext.data.StoreManager.lookup('overviewStore');
				var fromDate = this.down('#fromDate').getValue().toLocaleDateString()+"T00:00:00";
				var toDate = this.down('#toDate').getValue().toLocaleDateString()+"T23:59:59";
	...
	function createTimeFilter() {
		var dn = new Date(),
			lo = Ext.Date.format(Ext.Date.add(dn, Ext.Date.YEAR, -1), 'Y-m-d'); // Show one year of data initially.
			hi = Ext.Date.format(dn, 'Y-m-d');
		...
		return Ext.widget('toolbar', {
			...
			listeners: {
				loaded: function() {
					var tl = this.down('#timeline'),
						ll = this.down('#minlabel'),
						hl = this.down('#maxlabel'),
						dn = new Date();

					this.filterStore(function() { // Callback needed since 'data' is not set until first call to createStoreFeed is finished.
					...
					if(SO.defaultPeriod) {
                        var periods = {'Alla': '#all', 'Årets': '#thisYear', 'Månadens': '#thisMonth', '1 år': '#oneYear'};
			...
			filterStore: function(aCallback) {
				var sc = Ext.data.StoreManager.lookup('overviewStore'),
					tl = this.down('#timeline');
					lo = tl.getLoValue() + 'T00:00:00', // Time part is needed to include same day.
					hi = tl.getHiValue() + 'T23:59:59'; // Ditto for upper date limit.

				tl.setLoading('Filtrerar ...');
				Ext.Function.defer(function() { // To make loading mask show.
					sc.loadData(createStoreFeed(data.feed, lo, hi));
					tl.setLoading(false);
					if (aCallback) {
						aCallback();
					}
	...
	function createFilterFields() {
		...
		function processKeys(aField) {
			var	gp = aField.up('toolbar'),
				sc = Ext.data.StoreManager.lookup('overviewStore'),
				fv = aField.getValue() || undefined, // To match undefined lastQuery.
				fn = aField.getName().replace('filter',''),
				fa = Ext.Array.filter(sc.getFilters().items, function(aFilter) { return aFilter.getId() !== fn; }); //To reinstall all filters except the current one.
	...
	function createContentToDownload() {
        ...
        for(var i in data.data.items){
        	content += data.data.items[i].data['SubjectKey'].substring(8,21)+';';
        	for(var item in data.data.items[i].data){
        		if(Ext.String.startsWith(item,'F')){
					var value = data.data.items[i].data[item].replace(/<span.*?>/,'').replace('</span>', '').replace('&nbsp', ' ');
        			content += value +';';
        		}
        	}
        	content += '\n';
	...
	function createStoreFeed(anOverview, aMinDate, aMaxDate) {
		var gf = SO.repeatingLevel,
			ef = Repository.Global.Methods.EncryptNIN || function(x) { return x; },
			oa = [],
			lo = '9', // Higher than any date string in ISO8601 format.
			hi = '0', // Ditto for lowest.
			cr = 0,
			cg = 0,
			sk, ro, ie, rc;
	...
	SO.beforeProcess(function() {
		var tl = createTimeFilter(), 
			bs = createSummaryBar(),
			ff = createFilterFields(),
			dateFilter = createDateFilter(),
			afterrender = SO.afterProcess ? SO.afterProcess : function(){};
